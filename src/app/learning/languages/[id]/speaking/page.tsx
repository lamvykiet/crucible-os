...
export default function SpeakingPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { t } = useLanguage();
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24 space-y-6">
      ...
      <h1 className="c-h1">{t("Speaking", "Luyện nói")}</h1>
      {langCode ? (
        <SkillShell langCode={langCode} skill="speaking">
          <SpeakingPractice languageId={id} />
        </SkillShell>
      ) : (
        <SpeakingPractice languageId={id} />
      )}
